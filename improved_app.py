...
if uploaded_file:
    try:
        ...
        if "Keyword" not in df.columns:
            ...
            iterations = 0
            min_cluster_size = 5  # Initial minimum cluster size
            ...
            while True:
                if clustering_method == "Agglomerative":
                    ...
                    st.write(f"Iteration {iterations + 1}: Adjusted distance threshold: {distance_threshold:.2f}")
                    st.write(f"Iteration {iterations + 1}: Number of clusters formed: {n_clusters}")
                elif clustering_method == "K-means":
                    corpus_embeddings = model.encode(df['Keyword'].tolist(), batch_size=256, show_progress_bar=True)
                    clustering_model = KMeans(n_clusters=n_clusters)
                    cluster_labels = clustering_model.fit_predict(corpus_embeddings)
                    st.write(f"Iteration {iterations + 1}: Number of clusters formed: {n_clusters}")

                coherence_scores = calculate_cluster_coherence(corpus_embeddings, cluster_labels)
                overall_coherence = np.mean(coherence_scores)
                st.write(f"Iteration {iterations + 1}: Overall Clustering Coherence: {overall_coherence:.4f}")

                if overall_coherence < 0.5 or iterations > 5:  # Adjust coherence threshold as needed
                    break

                iterations += 1
                min_cluster_size -= 1  # Decrease min_cluster_size dynamically

            df['Cluster Name'] = cluster_labels
